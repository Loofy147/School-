...
- **Threat Modeling:** A proactive process where potential threats and vulnerabilities are identified, quantified, and mitigated during the design phase. A common framework for this is **STRIDE**, which stands for:
    ...
    - **E**levation of Privilege: Gaining higher-level access than authorized.
...
- **نمذجة التهديدات:** عملية استباقية يتم فيها تحديد التهديدات ونقاط الضعف المحتملة وقياسها وتخفيفها أثناء مرحلة التصميم. الإطار الشائع لهذا هو **STRIDE**، والذي يرمز إلى:
    - **S**poofing (الانتحال): انتحال شخصية مستخدم أو خدمة أخرى.
    - **T**ampering (العبث): تعديل البيانات دون إذن.
    - **R**epudiation (الإنكار): إنكار اتخاذ إجراء ما.
    - **I**nformation Disclosure (الكشف عن المعلومات): كشف البيانات الحساسة لأطراف غير مصرح لها.
    - **D**enial of Service (رفض الخدمة): جعل النظام غير متاح للمستخدمين الشرعيين.
    - **E**levation of Privilege (تصعيد الامتيازات): الحصول على وصول بمستوى أعلى من المصرح به.
